...
function PC.Send(targetUserId, text)
    ...
    table.insert(PC.Conversations[targetUserId].messages, { from = "me", text = text, ts = os.time() })
    req({
        Url = PC.Server .. "/send",
        Method = "POST",
        ...
        if res and res.Body then
            ...
            if ok and type(msgs) == "table" then
                for _, m in ipairs(msgs) do
                    ...
                    table.insert(PC.Conversations[uid].messages, { from = "them", text = m.payload, ts = os.time() })
...
do
    ...
    local function loadConversation(uid)
        ...
        task.delay(0.05, function()
            msgFrame.CanvasPosition = Vector2.new(0, math.max(0, msgFrame.CanvasSize.Y.Offset - msgFrame.AbsoluteWindowSize.Y))
        end)
    end
    ...
    local function rebuildConversationList()
        ...
        for uid in pairs(PC.Conversations) do
            ...
            pcall(function()
                btn.Text = Players:GetNameFromUserIdAsync(tonumber(uid))
            end)
